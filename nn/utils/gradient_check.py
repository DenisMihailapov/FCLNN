...
def check_model_gradient(model, X, y, param_name="W",
                         delta=1e-5, tol=1e-4):
    """
    ...
    for layer_key in params:
        print("Checking gradient for %s" % layer_key)
        param = params[layer_key]
        initial_w = param[param_name].value

        def helper_func(w):
            param[param_name].value = w
            model.zero_grad()
            model.predict(X)
            loss = model.compute_loss_and_gradients(y)
            grad = param[param_name].grad
            return loss, grad
